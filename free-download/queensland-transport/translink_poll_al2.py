...
# Older SQLite builds (RHEL 7 / Amazon Linux 2 ship 3.7.17) lack features this
# script would otherwise use. Detect once and degrade rather than crash.
SQLITE_VERSION = tuple(int(part) for part in sqlite3.sqlite_version.split("."))
HAS_WITHOUT_ROWID = SQLITE_VERSION >= (3, 8, 2)
HAS_WINDOW_FUNCTIONS = SQLITE_VERSION >= (3, 25, 0)
...
SUMMARY_SCAN_SQL = """
SELECT trip_id, route_id, vehicle_id, ts, lat, lon
FROM vehicle_position
WHERE trip_id IS NOT NULL
  AND lat IS NOT NULL
ORDER BY trip_id, vehicle_id, ts
"""
...
def connect(path):
    ...
    schema = SCHEMA_TEMPLATE.format(
        without_rowid=" WITHOUT ROWID" if HAS_WITHOUT_ROWID else "")
...
def summarise_by_scan(connection, limit):
    """Compute per-trip statistics in Python, for SQLite without window functions."""
    trips = {}
    previous_key = None
    previous = None

    for trip_id, route_id, vehicle_id, ts, lat, lon in connection.execute(
            SUMMARY_SCAN_SQL):
        key = (trip_id, route_id, vehicle_id)
        entry = trips.setdefault(key, {
            "points": 0, "first": ts, "last": ts, "metres": 0.0, "gap": 0.0})
        entry["points"] += 1
        entry["first"] = min(entry["first"], ts)
        entry["last"] = max(entry["last"], ts)

        if key == previous_key and previous is not None:
            step = haversine(previous[0], previous[1], lat, lon) or 0.0
            entry["metres"] += step
            entry["gap"] = max(entry["gap"], step)

        previous_key = key
        previous = (lat, lon)

    rows = []
    for (trip_id, route_id, vehicle_id), entry in trips.items():
        if entry["points"] < 2:
            continue
        rows.append((
            trip_id,
            ...
            vehicle_id,
            entry["points"],
            time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(entry["first"])),
            time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(entry["last"])),
            round((entry["last"] - entry["first"]) / 60.0, 1),
            round(entry["metres"] / 1000.0, 3),
            round(entry["gap"]),
        ))
...
def run_summary(connection, limit):
    ...
    if HAS_WINDOW_FUNCTIONS:
        rows = connection.execute(SUMMARY_WINDOW_SQL, (limit,)).fetchall()
    else:
        rows = summarise_by_scan(connection, limit)
    if not rows:
